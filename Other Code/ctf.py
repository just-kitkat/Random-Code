# This was for a ctf challenge but is coded in an outdated dpy library
...
async def dev(ctx):
  with Connect() as cnx:
        cursor = cnx.cursor()
        cursor.execute('CREATE TABLE users(username)')


@bot.check
async def globally_block_guild(ctx):
    return True #ctx.guild is None


@bot.command(description="Help me")
async def helps(ctx):
    help_message = \
"""
...
@bot.command(description="Register for an account or change your username")
...
@bot.command(description="whoami")
...
@bot.command(description="Beg for money")
async def beg(ctx):
    with Connect() as cnx:
        cursor = cnx.cursor()
        cursor.execute('select 1 from users where id = ?', (ctx.author.id,))
...
@bot.command(description="Window shopping")
async def shop(ctx):
    with Connect() as cnx:
        cursor = cnx.cursor()
...
@bot.command(description="See your inventory")
async def inv(ctx):
    with Connect() as cnx:
        cursor = cnx.cursor()
        cursor.execute("select money from users where id = ?", (ctx.author.id,))
